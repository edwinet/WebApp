...
public partial class WebApp {
    ...
    /// exportLogsSearch
    /// </summary>
    public static ExportLogsSearch exportLogsSearch
    {
        get => HttpData.Get<ExportLogsSearch>("exportLogsSearch")!;
        set => HttpData["exportLogsSearch"] = value;
    }

    /// <summary>
    /// Page class for export_logs
    /// </summary>
    public class ExportLogsSearch : ExportLogsSearchBase
    {
        // Constructor
        public ExportLogsSearch(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ExportLogsSearch() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class ExportLogsSearchBase : ExportLogs
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "exportLogsSearch";
        ...
        // Page name
        public string PageName => "exportlogssearch";
        ...
        // Constructor
        public ExportLogsSearchBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsSearch) {
                ...
                if (!Empty(srchStr)) {
                    srchStr = "exportlogslist?" + srchStr;
        ...
        protected string BuildAdvancedSearch() {
            string srchUrl = "";
            BuildSearchUrl(ref srchUrl, file_id); // file_id
            BuildSearchUrl(ref srchUrl, created_at); // created_at
            BuildSearchUrl(ref srchUrl, _username); // username
            BuildSearchUrl(ref srchUrl, export_type); // export_type
            BuildSearchUrl(ref srchUrl, entity); // entity
            BuildSearchUrl(ref srchUrl, key_value); // key_value
            BuildSearchUrl(ref srchUrl, filename); // filename
            BuildSearchUrl(ref srchUrl, _request); // request
            if (!Empty(srchUrl))
                srchUrl += "&";
            srchUrl += "cmd=search";
            return srchUrl;
        ...
        protected void LoadSearchValues() {
            // file_id
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_file_id"))
                    file_id.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_file_id");
            if (Form.ContainsKey("z_file_id"))
                file_id.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_file_id");
            ...
            // export_type
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_export_type"))
                    export_type.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_export_type");
            if (Form.ContainsKey("z_export_type"))
                export_type.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_export_type");
            ...
            // filename
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_filename"))
                    filename.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_filename");
            if (Form.ContainsKey("z_filename"))
                filename.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_filename");

            // _request
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x__request"))
                    _request.AdvancedSearch.SearchValue = CurrentForm.GetValue("x__request");
            if (Form.ContainsKey("z__request"))
                _request.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z__request");
        }
        ...
        public async Task RenderRow()
        {
            ...
            // filename
            filename.RowCssClass = "row";

            // request
            _request.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                file_id.EditValue = file_id.AdvancedSearch.SearchValue; // DN
                ...
                if (!export_type.Raw)
                    export_type.AdvancedSearch.SearchValue = HtmlDecode(export_type.AdvancedSearch.SearchValue);
                export_type.EditValue = HtmlEncode(export_type.AdvancedSearch.SearchValue);
                ...
                if (!key_value.Raw)
                    key_value.AdvancedSearch.SearchValue = HtmlDecode(key_value.AdvancedSearch.SearchValue);
                key_value.EditValue = HtmlEncode(key_value.AdvancedSearch.SearchValue);
                key_value.PlaceHolder = RemoveHtml(key_value.Caption);

                // filename
                filename.SetupEditAttributes();
                if (!filename.Raw)
                    filename.AdvancedSearch.SearchValue = HtmlDecode(filename.AdvancedSearch.SearchValue);
                filename.EditValue = HtmlEncode(filename.AdvancedSearch.SearchValue);
                filename.PlaceHolder = RemoveHtml(filename.Caption);

                // request
                _request.SetupEditAttributes();
                _request.EditValue = _request.AdvancedSearch.SearchValue; // DN
        ...
        protected bool ValidateSearch() {
            ...
            if (!Config.ServerValidate)
                return true;
            if (!CheckGuid(ConvertToString(file_id.AdvancedSearch.SearchValue))) {
                file_id.AddErrorMessage(file_id.GetErrorMessage(false));
            }
