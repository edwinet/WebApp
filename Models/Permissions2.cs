...
public partial class WebApp {
    ...
    /// </summary>
    [MaybeNull]
    public static Permissions2 permissions2
    {
        get => HttpData.Resolve<Permissions2>("permissions2");
        set => HttpData["permissions2"] = value;
    }

    /// <summary>
    /// Table class for permissions
    /// </summary>
    public class Permissions2 : DbTable, IQueryFactory
    {
        ...
        public Permissions2()
        {
            ...
            Name = "permissions";
            Type = "TABLE";
            UpdateTable = "dbo.permissions"; // Update Table
            DbId = "DB"; // DN
            ExportAll = false;
            ...
            entity = new (this, "x_entity", 202, SqlDbType.NVarChar) {
                ...
                CustomMessage = Language.FieldPhrase("permissions2", "entity", "CustomMsg"),
                IsUpload = false
            };
            ...
            role_id = new (this, "x_role_id", 3, SqlDbType.Int) {
                ...
                CustomMessage = Language.FieldPhrase("permissions2", "role_id", "CustomMsg"),
                IsUpload = false
            };
            ...
            permission = new (this, "x_permission", 3, SqlDbType.Int) {
                ...
                DefaultErrorMessage = Language.Phrase("IncorrectInteger"),
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN" },
                CustomMessage = Language.FieldPhrase("permissions2", "permission", "CustomMsg"),
                IsUpload = false
            };
        ...
        private string _sqlKeyFilter => "[entity] = '@entity@' AND [role_id] = @role_id@";
        ...
        public string GetRecordFilter(Dictionary<string, object>? row = null, bool current = false)
        {
            ...
            val = row?.TryGetValue("entity", out result) ?? false
                ? result
                : !Empty(entity.OldValue) && !current ? entity.OldValue : entity.CurrentValue;
            if (val == null)
                return "0=1"; // Invalid key
            keyFilter = keyFilter.Replace("@entity@", AdjustSql(val, DbId)); // Replace key value
            val = row?.TryGetValue("role_id", out result) ?? false
                ? result
                : !Empty(role_id.OldValue) && !current ? role_id.OldValue : role_id.CurrentValue;
            if (!IsNumeric(val))
                return "0=1"; // Invalid key
            if (val == null)
                return "0=1"; // Invalid key
            keyFilter = keyFilter.Replace("@role_id@", AdjustSql(val, DbId)); // Replace key value
            return keyFilter;
        }
        ...
        public Dictionary<string, object>? GetRowFilterAsDictionary(IDictionary<string, object>? row = null)
        {
            ...
            val = row?.TryGetValue("entity", out result) ?? false
                ? result
                : !Empty(entity.OldValue) ? entity.OldValue : entity.CurrentValue;
            if (Empty(val))
                return null; // Invalid key
            keyFilter.Add("entity", val); // Add key value
            val = row?.TryGetValue("role_id", out result) ?? false
                ? result
                : !Empty(role_id.OldValue) ? role_id.OldValue : role_id.CurrentValue;
            if (!IsNumeric(val))
                return null; // Invalid key
            if (Empty(val))
                return null; // Invalid key
            keyFilter.Add("role_id", val); // Add key value
        ...
        {
            if (SameString(pageName, "permissions2view"))
                return Language.Phrase("View");
            else if (SameString(pageName, "permissions2edit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "permissions2add"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "Permissions2View",
                Config.ApiAddAction => "Permissions2Add",
                Config.ApiEditAction => "Permissions2Edit",
                Config.ApiDeleteAction => "Permissions2Delete",
                Config.ApiListAction => "Permissions2List",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "permissions2list";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("permissions2view", parm);
            else
                url = KeyUrl("permissions2view", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "permissions2add";
        ...
        {
            string url = "";
            url = KeyUrl("permissions2edit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("permissions2list", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("permissions2add", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("permissions2list", "action=copy"))); // DN

        // Delete URL
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ? AppPath(KeyUrl(Config.ApiUrl + Config.ApiDeleteAction + "/" + TableVar))
            : AppPath(KeyUrl("permissions2delete")); // DN
        ...
        {
            string json = "";
            json += "\"entity\":" + ConvertToJson(entity.CurrentValue, "string", true);
            json += ",\"role_id\":" + ConvertToJson(role_id.CurrentValue, "number", true);
            json = "{" + json + "}";
            if (htmlEncode)
        ...
        public string KeyUrl(string url, string parm = "") { // DN
            if (!IsNull(entity.CurrentValue)) {
                url += "/" + EncodeKeyValue(ConvertToString(entity.CurrentValue));
            } else {
                return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
            }
            if (!IsNull(role_id.CurrentValue)) {
                url += "/" + role_id.CurrentValue;
            } else {
                return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
            }
            if (Empty(parm))
        ...
        public string GetKey(bool current = false)
        {
            ...
            string val;
            val = current ? ConvertToString(entity.CurrentValue) ?? "" : ConvertToString(entity.OldValue) ?? "";
            if (Empty(val))
                return String.Empty;
            keys.Add(val);
            val = current ? ConvertToString(role_id.CurrentValue) ?? "" : ConvertToString(role_id.OldValue) ?? "";
            if (Empty(val))
                return String.Empty;
            keys.Add(val);
        ...
        public string GetKey(IDictionary<string, object> row)
        {
            List<string> keys = new ();
            object? val = null, result;
            val = row?.TryGetValue("entity", out result) ?? false ? ConvertToString(result) : null;
            if (Empty(val))
                return String.Empty; // Invalid key
            keys.Add(ConvertToString(val)); // Add key value
            val = row?.TryGetValue("role_id", out result) ?? false ? ConvertToString(result) : null;
            if (Empty(val))
                return String.Empty; // Invalid key
            keys.Add(ConvertToString(val)); // Add key value
            return String.Join(Config.CompositeKeySeparator, keys);
        ...
        public void SetKey(string key, bool current = false)
        {
            ...
            if (keys.Length == 2) {
                if (current) {
                    entity.CurrentValue = keys[0];
                } else {
                    entity.OldValue = keys[0];
                }
                if (current) {
                    role_id.CurrentValue = keys[1];
                } else {
                    role_id.OldValue = keys[1];
                }
            }
        }
        ...
        public List<string[]> GetRecordKeys()
        {
            ...
            if (Post("key_m[]", out sv) || Get("key_m[]", out sv)) { // DN
                ...
                if (RouteValues.TryGetValue("entity", out object? v0)) { // entity // DN
                    key = UrlDecode(v0); // DN
                ...
                keys.Add(key);
                if (RouteValues.TryGetValue("role_id", out object? v1)) { // role_id // DN
                    key = UrlDecode(v1); // DN
                } else if (IsApi() && !Empty(keyValues)) {
                    key = keyValues[1];
                } else {
        ...
        public string GetFilterFromRecordKeys(bool setCurrent = true)
        {
            ...
            foreach (var keys in recordKeys) {
                if (!Empty(keyFilter))
                    keyFilter += " OR ";
                if (setCurrent)
                    entity.CurrentValue = ((string[])keys)[0];
                else
                    entity.OldValue = ((string[])keys)[0];
                if (setCurrent)
                    role_id.CurrentValue = ((string[])keys)[1];
                else
                    role_id.OldValue = ((string[])keys)[1];
        ...
        {
            string pageName = "Permissions2List";
        ...
        public async Task RenderEditRow()
        {
            ...
            role_id.EditValue = role_id.CurrentValue; // DN
