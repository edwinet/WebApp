...
public partial class WebApp {
    ...
    /// auditTrailsSearch
    /// </summary>
    public static AuditTrailsSearch auditTrailsSearch
    {
        get => HttpData.Get<AuditTrailsSearch>("auditTrailsSearch")!;
        set => HttpData["auditTrailsSearch"] = value;
    }

    /// <summary>
    /// Page class for audit_trails
    /// </summary>
    public class AuditTrailsSearch : AuditTrailsSearchBase
    {
        // Constructor
        public AuditTrailsSearch(Controller controller) : base(controller)
        {
        }

        // Constructor
        public AuditTrailsSearch() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class AuditTrailsSearchBase : AuditTrails
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "auditTrailsSearch";
        ...
        // Page name
        public string PageName => "audittrailssearch";
        ...
        // Constructor
        public AuditTrailsSearchBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsSearch) {
                ...
                if (!Empty(srchStr)) {
                    srchStr = "audittrailslist?" + srchStr;
        ...
        protected string BuildAdvancedSearch() {
            ...
            BuildSearchUrl(ref srchUrl, script); // script
            BuildSearchUrl(ref srchUrl, _username); // username
            BuildSearchUrl(ref srchUrl, _action); // action
            BuildSearchUrl(ref srchUrl, entity); // entity
            BuildSearchUrl(ref srchUrl, _field); // field
            BuildSearchUrl(ref srchUrl, key_value); // key_value
            BuildSearchUrl(ref srchUrl, old_value); // old_value
            BuildSearchUrl(ref srchUrl, new_value); // new_value
            if (!Empty(srchUrl))
                srchUrl += "&";
            srchUrl += "cmd=search";
            return srchUrl;
        ...
        protected void LoadSearchValues() {
            ...
            // script
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_script"))
                    script.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_script");
            if (Form.ContainsKey("z_script"))
                script.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_script");
            ...
            // entity
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_entity"))
                    entity.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_entity");
            if (Form.ContainsKey("z_entity"))
                entity.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_entity");

            // _field
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x__field"))
                    _field.AdvancedSearch.SearchValue = CurrentForm.GetValue("x__field");
            if (Form.ContainsKey("z__field"))
                _field.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z__field");

            // key_value
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_key_value"))
                    key_value.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_key_value");
            if (Form.ContainsKey("z_key_value"))
                key_value.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_key_value");

            // old_value
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_old_value"))
                    old_value.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_old_value");
            if (Form.ContainsKey("z_old_value"))
                old_value.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_old_value");

            // new_value
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_new_value"))
                    new_value.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_new_value");
            if (Form.ContainsKey("z_new_value"))
                new_value.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_new_value");
        }
        ...
        public async Task RenderRow()
        {
            ...
            // key_value
            key_value.RowCssClass = "row";

            // old_value
            old_value.RowCssClass = "row";

            // new_value
            new_value.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                if (!script.Raw)
                    script.AdvancedSearch.SearchValue = HtmlDecode(script.AdvancedSearch.SearchValue);
                script.EditValue = HtmlEncode(script.AdvancedSearch.SearchValue);
                ...
                if (!entity.Raw)
                    entity.AdvancedSearch.SearchValue = HtmlDecode(entity.AdvancedSearch.SearchValue);
                entity.EditValue = HtmlEncode(entity.AdvancedSearch.SearchValue);
                entity.PlaceHolder = RemoveHtml(entity.Caption);

                // field
                _field.SetupEditAttributes();
                if (!_field.Raw)
                    _field.AdvancedSearch.SearchValue = HtmlDecode(_field.AdvancedSearch.SearchValue);
                _field.EditValue = HtmlEncode(_field.AdvancedSearch.SearchValue);
                _field.PlaceHolder = RemoveHtml(_field.Caption);

                // key_value
                key_value.SetupEditAttributes();
                key_value.EditValue = key_value.AdvancedSearch.SearchValue; // DN
                key_value.PlaceHolder = RemoveHtml(key_value.Caption);

                // old_value
                old_value.SetupEditAttributes();
                old_value.EditValue = old_value.AdvancedSearch.SearchValue; // DN
                old_value.PlaceHolder = RemoveHtml(old_value.Caption);

                // new_value
                new_value.SetupEditAttributes();
                new_value.EditValue = new_value.AdvancedSearch.SearchValue; // DN
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("audittrailslist")), "", TableVar, true);
