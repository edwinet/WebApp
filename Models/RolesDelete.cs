...
public partial class WebApp {
    ...
    /// rolesDelete
    /// </summary>
    public static RolesDelete rolesDelete
    {
        get => HttpData.Get<RolesDelete>("rolesDelete")!;
        set => HttpData["rolesDelete"] = value;
    }

    /// <summary>
    /// Page class for roles
    /// </summary>
    public class RolesDelete : RolesDeleteBase
    {
        // Constructor
        public RolesDelete(Controller controller) : base(controller)
        {
        }

        // Constructor
        public RolesDelete() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class RolesDeleteBase : Roles
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "rolesDelete";
        ...
        // Page name
        public string PageName => "rolesdelete";
        ...
        // Constructor
        public RolesDeleteBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (Empty(filter))
                return Terminate("roleslist"); // Prevent SQL injection, return to List page
            ...
            if (IsShow) { // Load records for display // DN
                ...
                if (TotalRecords <= 0) { // No record found, exit
                    CloseRecordset(); // DN
                    return Terminate("roleslist"); // Return to list
                }
            }
                ...
                if (result) {
                    foreach (var row in rsold) {
                        ...
                        if (!result) {
                            ...
                            if (!Empty(row["id"]))
                                await Resolve(Config.UserLevelPrivTableName)?.DeleteAsync(null, new Dictionary<string, object> { { Config.UserLevelPrivUserLevelIdField, ConvertToInt(row["id"]) } }); // Delete user level permissions (Use main connection)
                        }
                    }
                }
