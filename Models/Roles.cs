...
public partial class WebApp {
    ...
    {
        get => HttpData.Resolve<Roles>("roles");
        set => HttpData["roles"] = value;
    }

    /// <summary>
    /// Table class for roles
    /// </summary>
    public class Roles : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<SqlDbType> role_name;

        public readonly DbField<SqlDbType> description;

        public readonly DbField<SqlDbType> _type;
        ...
        public Roles()
        {
            ...
            Name = "roles";
            Type = "TABLE";
            UpdateTable = "dbo.roles"; // Update Table
            DbId = "DB"; // DN
            ExportAll = false;
            ...
            id = new (this, "x_id", 3, SqlDbType.Int) {
                ...
                CustomMessage = Language.FieldPhrase("roles", "id", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("id", id);

            // role_name
            role_name = new (this, "x_role_name", 202, SqlDbType.NVarChar) {
                Name = "role_name",
                Expression = "[role_name]",
                UseBasicSearch = true,
                BasicSearchExpression = "[role_name]",
                DateTimeFormat = -1,
                VirtualExpression = "[role_name]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("roles", "role_name", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("role_name", role_name);
            ...
            description = new (this, "x_description", 202, SqlDbType.NVarChar) {
                ...
                CustomMessage = Language.FieldPhrase("roles", "description", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("description", description);

            // type
            _type = new (this, "x__type", 202, SqlDbType.NVarChar) {
                Name = "type",
                Expression = "[type]",
                UseBasicSearch = true,
                BasicSearchExpression = "[type]",
                DateTimeFormat = -1,
                VirtualExpression = "[type]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                InputTextType = "text",
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("roles", "_type", "CustomMsg"),
                IsUpload = false
            };
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                role_name.DbValue = row["role_name"]; // Set DB value only
                description.DbValue = row["description"]; // Set DB value only
                _type.DbValue = row["type"]; // Set DB value only
            } catch {}
        }
        ...
        {
            if (SameString(pageName, "rolesview"))
                return Language.Phrase("View");
            else if (SameString(pageName, "rolesedit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "rolesadd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "RolesView",
                Config.ApiAddAction => "RolesAdd",
                Config.ApiEditAction => "RolesEdit",
                Config.ApiDeleteAction => "RolesDelete",
                Config.ApiListAction => "RolesList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "roleslist";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("rolesview", parm);
            else
                url = KeyUrl("rolesview", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "rolesadd";
        ...
        {
            string url = "";
            url = KeyUrl("rolesedit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("roleslist", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("rolesadd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("roleslist", "action=copy"))); // DN

        // Delete URL
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ? AppPath(KeyUrl(Config.ApiUrl + Config.ApiDeleteAction + "/" + TableVar))
            : AppPath(KeyUrl("rolesdelete")); // DN
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            if (dr == null)
                return;
            id.SetDbValue(dr["id"]);
            role_name.SetDbValue(dr["role_name"]);
            description.SetDbValue(dr["description"]);
            _type.SetDbValue(dr["type"]);
        }

        // Render list content
        public async Task<string> RenderListContent(string filter)
        {
            string pageName = "RolesList";
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    ...
                    if (exportType == "view") {
                        ...
                        doc.ExportCaption(role_name);
                        doc.ExportCaption(description);
                        doc.ExportCaption(_type);
                    } else {
                        doc.ExportCaption(id);
                        doc.ExportCaption(role_name);
                        doc.ExportCaption(description);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(role_name);
                            await doc.ExportField(description);
                            await doc.ExportField(_type);
                        } else {
                            await doc.ExportField(id);
                            await doc.ExportField(role_name);
                            await doc.ExportField(description);
        ...
        public async Task WriteAuditTrailLog(string log) => await WriteAuditTrailAsync(CurrentUser(), log, "roles");
