...
public partial class WebApp {
    ...
    /// rolesList
    /// </summary>
    public static RolesList rolesList
    {
        get => HttpData.Get<RolesList>("rolesList")!;
        set => HttpData["rolesList"] = value;
    }

    /// <summary>
    /// Page class for roles
    /// </summary>
    public class RolesList : RolesListBase
    {
        // Constructor
        public RolesList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public RolesList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class RolesListBase : Roles
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "rolesList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "froleslist";
        ...
        public RolesListBase()
        {
            ...
            MultiDeleteUrl = "rolesdelete";
            MultiUpdateUrl = "rolesupdate";
        ...
        // Page name
        public string PageName => "roleslist";
        ...
        // Constructor
        public RolesListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(id); // id
                UpdateSort(role_name); // role_name
                UpdateSort(description); // description
                UpdateSort(_type); // type
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""roles"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"froleslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"froleslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            if (Security.HasUserLevelID(ConvertToInt(id.CurrentValue)) || ConvertToInt(id.CurrentValue) < 0 && ConvertToInt(id.CurrentValue) != -2) {
                listOption?.SetBody("-");
            } else {
                listOption?.SetBody("<a class=\"ew-row-link ew-user-permission\" title=\"" + Language.Phrase("Permission", true) + "\" data-caption=\"" + Language.Phrase("Permission", true) + "\" href=\"" + HtmlEncode(AppPath("userpriv?id=" + id.CurrentValue)) + "\">" + Language.Phrase("Permission") + "</a>");
            }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""roles"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("role_name")); // DN
                CreateColumnOption(option.Add("description")); // DN
                CreateColumnOption(option.Add("type")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"frolessrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = false;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"frolessrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = false;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"froleslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(rolesList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(rolesList.RowCount) + "_roles");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(rolesList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && rolesList.RowType == RowType.Add || IsEdit && rolesList.RowType == RowType.Edit) // Inline-Add/Edit row
