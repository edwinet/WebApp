...
public partial class WebApp {
    ...
    /// restaurantsAdd
    /// </summary>
    public static RestaurantsAdd restaurantsAdd
    {
        get => HttpData.Get<RestaurantsAdd>("restaurantsAdd")!;
        set => HttpData["restaurantsAdd"] = value;
    }

    /// <summary>
    /// Page class for restaurants
    /// </summary>
    public class RestaurantsAdd : RestaurantsAddBase
    {
        // Constructor
        public RestaurantsAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public RestaurantsAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class RestaurantsAddBase : Restaurants
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "restaurantsAdd";
        ...
        // Page name
        public string PageName => "restaurantsadd";
        ...
        // Constructor
        public RestaurantsAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("restaurantslist"); // No matching record, return to List page // DN
                    }
                ...
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        else if (GetPageName(returnUrl) == "restaurantsview")
        ...
        protected async Task<bool> ValidateForm() {
            ...
            if (detailTblVar.Contains("dishes") && dishes?.DetailAdd == true) {
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            if (!Empty(_name.CurrentValue)) { // Check field with unique index
                var filter = "(name = '" + AdjustSql(_name.CurrentValue, DbId) + "')";
                using var rschk = await LoadReader(filter);
                if (rschk?.HasRows ?? false) { // DN
                    FailureMessage = Language.Phrase("DupIndex").Replace("%f", _name.Caption).Replace("%v", ConvertToString(_name.CurrentValue));
                    return JsonBoolResult.FalseResult;
                }
            }
            ...
            // Add detail records
            var detailTblVar = CurrentDetailTables;
            if (detailTblVar.Contains("dishes") && dishes?.DetailAdd == true && result) {
                dishes.restaurant_id.SessionValue = ConvertToString(id.CurrentValue); // Set master key
                dishesGrid = Resolve("DishesGrid")!; // Get detail page object
                if (dishesGrid != null) {
                    Security.LoadCurrentUserLevel(ProjectID + "dishes"); // Load user level of detail table
                    result = await dishesGrid.GridInsert();
        ...
        protected void SetupDetailParms() {
            ...
            if (!Empty(detailTblVar)) {
                ...
                if (detailTblVars.Contains("dishes")) {
                    ...
                    if (dishesGrid?.DetailAdd ?? false) {
                        if (CopyRecord)
                            dishesGrid.CurrentMode = "copy";
                        else
                            dishesGrid.CurrentMode = "add";
                        dishesGrid.CurrentAction = "gridadd";
