...
public partial class WebApp {
    ...
    /// exportLogsList
    /// </summary>
    public static ExportLogsList exportLogsList
    {
        get => HttpData.Get<ExportLogsList>("exportLogsList")!;
        set => HttpData["exportLogsList"] = value;
    }

    /// <summary>
    /// Page class for export_logs
    /// </summary>
    public class ExportLogsList : ExportLogsListBase
    {
        // Constructor
        public ExportLogsList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public ExportLogsList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class ExportLogsListBase : ExportLogs
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "exportLogsList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fexport_logslist";
        ...
        // Constructor
        public ExportLogsListBase()
        {
            // CSS class name as context
            TableVar = "export_logs";
            ...
            MultiDeleteUrl = "exportlogsdelete";
            MultiUpdateUrl = "exportlogsupdate";
        ...
        // Page name
        public string PageName => "exportlogslist";
        ...
        // Constructor
        public ExportLogsListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fexport_logssrch");

            // Initialize
            var filters = new JObject(); // DN
            filters.Merge(JObject.Parse(file_id.AdvancedSearch.ToJson())); // Field file_id
            ...
            filters.Merge(JObject.Parse(export_type.AdvancedSearch.ToJson())); // Field export_type
            ...
            filters.Merge(JObject.Parse(_request.AdvancedSearch.ToJson())); // Field request
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fexport_logssrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field file_id
            if (filter?.TryGetValue("x_file_id", out sv) ?? false) {
                file_id.AdvancedSearch.SearchValue = sv;
                file_id.AdvancedSearch.SearchOperator = filter["z_file_id"];
                file_id.AdvancedSearch.SearchCondition = filter["v_file_id"];
                file_id.AdvancedSearch.SearchValue2 = filter["y_file_id"];
                file_id.AdvancedSearch.SearchOperator2 = filter["w_file_id"];
                file_id.AdvancedSearch.Save();
            }
            ...
            // Field export_type
            if (filter?.TryGetValue("x_export_type", out sv) ?? false) {
                export_type.AdvancedSearch.SearchValue = sv;
                export_type.AdvancedSearch.SearchOperator = filter["z_export_type"];
                export_type.AdvancedSearch.SearchCondition = filter["v_export_type"];
                export_type.AdvancedSearch.SearchValue2 = filter["y_export_type"];
                export_type.AdvancedSearch.SearchOperator2 = filter["w_export_type"];
                export_type.AdvancedSearch.Save();
            }
            ...
            // Field filename
            if (filter?.TryGetValue("x_filename", out sv) ?? false) {
            ...
            // Field request
            if (filter?.TryGetValue("x__request", out sv) ?? false) {
                _request.AdvancedSearch.SearchValue = sv;
                _request.AdvancedSearch.SearchOperator = filter["z__request"];
                _request.AdvancedSearch.SearchCondition = filter["v__request"];
                _request.AdvancedSearch.SearchValue2 = filter["y__request"];
                _request.AdvancedSearch.SearchOperator2 = filter["w__request"];
                _request.AdvancedSearch.Save();
            }
        ...
        public void ShowFilterList()
        {
            ...
            // Field file_id
            filter = QueryBuilderWhere("file_id");
            if (Empty(filter))
                BuildSearchSql(ref filter, file_id, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + file_id.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field export_type
            filter = QueryBuilderWhere("export_type");
            if (Empty(filter))
                BuildSearchSql(ref filter, export_type, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + export_type.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field filename
            filter = QueryBuilderWhere("filename");
            if (Empty(filter))
                BuildSearchSql(ref filter, filename, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + filename.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field request
            filter = QueryBuilderWhere("request");
            if (Empty(filter))
                BuildSearchSql(ref filter, _request, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + _request.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(export_type);
            searchFlds.Add(entity);
            searchFlds.Add(key_value);
            searchFlds.Add(filename);
            searchFlds.Add(_request);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (file_id.AdvancedSearch.IssetSession)
                return true;
            if (created_at.AdvancedSearch.IssetSession)
                return true;
            if (_username.AdvancedSearch.IssetSession)
                return true;
            if (export_type.AdvancedSearch.IssetSession)
                return true;
            if (entity.AdvancedSearch.IssetSession)
                return true;
            if (key_value.AdvancedSearch.IssetSession)
                return true;
            if (filename.AdvancedSearch.IssetSession)
                return true;
            if (_request.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            file_id.AdvancedSearch.UnsetSession();
            created_at.AdvancedSearch.UnsetSession();
            _username.AdvancedSearch.UnsetSession();
            export_type.AdvancedSearch.UnsetSession();
            entity.AdvancedSearch.UnsetSession();
            key_value.AdvancedSearch.UnsetSession();
            filename.AdvancedSearch.UnsetSession();
            _request.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            file_id.AdvancedSearch.Load();
            created_at.AdvancedSearch.Load();
            _username.AdvancedSearch.Load();
            export_type.AdvancedSearch.Load();
            entity.AdvancedSearch.Load();
            key_value.AdvancedSearch.Load();
            filename.AdvancedSearch.Load();
            _request.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = created_at.Expression + " DESC"; // Set up default sort
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""export_logs"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fexport_logslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fexport_logslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(file_id.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("file_id")); // DN
                CreateColumnOption(option.Add("created_at")); // DN
                CreateColumnOption(option.Add("username")); // DN
                CreateColumnOption(option.Add("export_type")); // DN
                CreateColumnOption(option.Add("entity")); // DN
                CreateColumnOption(option.Add("key_value")); // DN
                CreateColumnOption(option.Add("filename")); // DN
                CreateColumnOption(option.Add("request")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fexport_logssrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fexport_logssrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fexport_logslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_export_logs");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(exportLogsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(exportLogsList.RowCount) + "_export_logs");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(exportLogsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && exportLogsList.RowType == RowType.Add || IsEdit && exportLogsList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // file_id
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_file_id"))
                    file_id.AdvancedSearch.SearchValue = Get("x_file_id");
                else
                    file_id.AdvancedSearch.SearchValue = Get("file_id"); // Default Value // DN
            if (!Empty(file_id.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_file_id"))
                file_id.AdvancedSearch.SearchOperator = Get("z_file_id");
            ...
            // export_type
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_export_type"))
                    export_type.AdvancedSearch.SearchValue = Get("x_export_type");
                else
                    export_type.AdvancedSearch.SearchValue = Get("export_type"); // Default Value // DN
            if (!Empty(export_type.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_export_type"))
                export_type.AdvancedSearch.SearchOperator = Get("z_export_type");
            ...
            // filename
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_filename"))
                    filename.AdvancedSearch.SearchValue = Get("x_filename");
                else
                    filename.AdvancedSearch.SearchValue = Get("filename"); // Default Value // DN
            if (!Empty(filename.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_filename"))
                filename.AdvancedSearch.SearchOperator = Get("z_filename");

            // _request
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x__request"))
                    _request.AdvancedSearch.SearchValue = Get("x__request");
                else
                    _request.AdvancedSearch.SearchValue = Get("_request"); // Default Value // DN
            if (!Empty(_request.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z__request"))
                _request.AdvancedSearch.SearchOperator = Get("z__request");
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // request
                _request.ViewValue = TruncateMemo(ConvertToString(_request.CurrentValue), 255, _request.TruncateMemoRemoveHtml);
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fexport_logssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"export_logs\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("exportlogssearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("exportlogssearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
