...
public partial class WebApp {
    ...
    public class UsersListBase : Users
    {
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(first_name.AdvancedSearch.ToJson())); // Field first_name
            filters.Merge(JObject.Parse(last_name.AdvancedSearch.ToJson())); // Field last_name
            ...
            filters.Merge(JObject.Parse(mobile.AdvancedSearch.ToJson())); // Field mobile
            filters.Merge(JObject.Parse(role_id.AdvancedSearch.ToJson())); // Field role_id
        ...
        protected bool RestoreFilterList() {
            ...
            // Field first_name
            if (filter?.TryGetValue("x_first_name", out sv) ?? false) {
                first_name.AdvancedSearch.SearchValue = sv;
                first_name.AdvancedSearch.SearchOperator = filter["z_first_name"];
                first_name.AdvancedSearch.SearchCondition = filter["v_first_name"];
                first_name.AdvancedSearch.SearchValue2 = filter["y_first_name"];
                first_name.AdvancedSearch.SearchOperator2 = filter["w_first_name"];
                first_name.AdvancedSearch.Save();
            }

            // Field last_name
            if (filter?.TryGetValue("x_last_name", out sv) ?? false) {
                last_name.AdvancedSearch.SearchValue = sv;
                last_name.AdvancedSearch.SearchOperator = filter["z_last_name"];
                last_name.AdvancedSearch.SearchCondition = filter["v_last_name"];
                last_name.AdvancedSearch.SearchValue2 = filter["y_last_name"];
                last_name.AdvancedSearch.SearchOperator2 = filter["w_last_name"];
                last_name.AdvancedSearch.Save();
            }
            ...
            // Field mobile
            if (filter?.TryGetValue("x_mobile", out sv) ?? false) {
                mobile.AdvancedSearch.SearchValue = sv;
            ...
            // Field role_id
            if (filter?.TryGetValue("x_role_id", out sv) ?? false) {
                role_id.AdvancedSearch.SearchValue = sv;
                role_id.AdvancedSearch.SearchOperator = filter["z_role_id"];
                role_id.AdvancedSearch.SearchCondition = filter["v_role_id"];
                role_id.AdvancedSearch.SearchValue2 = filter["y_role_id"];
                role_id.AdvancedSearch.SearchOperator2 = filter["w_role_id"];
                role_id.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, first_name, def, false); // first_name
            BuildSearchSql(ref where, last_name, def, false); // last_name
            BuildSearchSql(ref where, _email, def, false); // _email
            BuildSearchSql(ref where, _username, def, false); // _username
            BuildSearchSql(ref where, mobile, def, false); // mobile
            BuildSearchSql(ref where, role_id, def, false); // role_id
            ...
            if (!def && Command == "search") {
                ...
                first_name.AdvancedSearch.Save(); // first_name
                last_name.AdvancedSearch.Save(); // last_name
                _email.AdvancedSearch.Save(); // email
                _username.AdvancedSearch.Save(); // username
                mobile.AdvancedSearch.Save(); // mobile
                role_id.AdvancedSearch.Save(); // role_id
        ...
        public void ShowFilterList()
        {
            ...
            // Field first_name
            filter = QueryBuilderWhere("first_name");
            if (Empty(filter))
                BuildSearchSql(ref filter, first_name, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + first_name.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field last_name
            filter = QueryBuilderWhere("last_name");
            if (Empty(filter))
                BuildSearchSql(ref filter, last_name, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + last_name.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field mobile
            filter = QueryBuilderWhere("mobile");
            if (Empty(filter))
                BuildSearchSql(ref filter, mobile, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + mobile.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(first_name);
            searchFlds.Add(last_name);
            searchFlds.Add(_email);
            searchFlds.Add(_username);
            searchFlds.Add(mobile);
            searchFlds.Add(photo);
        ...
        protected bool CheckSearchParms() {
            ...
            if (id.AdvancedSearch.IssetSession)
                return true;
            if (first_name.AdvancedSearch.IssetSession)
                return true;
            if (last_name.AdvancedSearch.IssetSession)
                return true;
            ...
            if (mobile.AdvancedSearch.IssetSession)
                return true;
            if (role_id.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            first_name.AdvancedSearch.UnsetSession();
            last_name.AdvancedSearch.UnsetSession();
            _email.AdvancedSearch.UnsetSession();
            _username.AdvancedSearch.UnsetSession();
            mobile.AdvancedSearch.UnsetSession();
            role_id.AdvancedSearch.UnsetSession();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(id); // id
                UpdateSort(first_name); // first_name
                UpdateSort(last_name); // last_name
                UpdateSort(_email); // email
                UpdateSort(_username); // username
                UpdateSort(mobile); // mobile
                UpdateSort(photo); // photo
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("first_name")); // DN
                CreateColumnOption(option.Add("last_name")); // DN
                CreateColumnOption(option.Add("email")); // DN
                CreateColumnOption(option.Add("username")); // DN
                CreateColumnOption(option.Add("mobile")); // DN
                CreateColumnOption(option.Add("photo")); // DN
        ...
        protected void LoadSearchValues() {
            ...
            // first_name
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_first_name"))
                    first_name.AdvancedSearch.SearchValue = Get("x_first_name");
                else
                    first_name.AdvancedSearch.SearchValue = Get("first_name"); // Default Value // DN
            if (!Empty(first_name.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_first_name"))
                first_name.AdvancedSearch.SearchOperator = Get("z_first_name");

            // last_name
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_last_name"))
                    last_name.AdvancedSearch.SearchValue = Get("x_last_name");
                else
                    last_name.AdvancedSearch.SearchValue = Get("last_name"); // Default Value // DN
            if (!Empty(last_name.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_last_name"))
                last_name.AdvancedSearch.SearchOperator = Get("z_last_name");
            ...
            // mobile
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_mobile"))
                    mobile.AdvancedSearch.SearchValue = Get("x_mobile");
                else
                    mobile.AdvancedSearch.SearchValue = Get("mobile"); // Default Value // DN
            if (!Empty(mobile.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_mobile"))
                mobile.AdvancedSearch.SearchOperator = Get("z_mobile");

            // role_id
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_role_id"))
                    role_id.AdvancedSearch.SearchValue = Get("x_role_id");
                else
                    role_id.AdvancedSearch.SearchValue = Get("role_id"); // Default Value // DN
            if (!Empty(role_id.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_role_id"))
                role_id.AdvancedSearch.SearchOperator = Get("z_role_id");
            ...
            if (Query.ContainsKey("v_created_at"))
                created_at.AdvancedSearch.SearchCondition = Get("v_created_at");
            if (Query.ContainsKey("y_created_at"))
                created_at.AdvancedSearch.SearchValue2 = Get("y_created_at");
            if (!Empty(created_at.AdvancedSearch.SearchValue2) && Command == "")
                Command = "search";
            if (Query.ContainsKey("w_created_at"))
                created_at.AdvancedSearch.SearchOperator2 = Get("w_created_at");
            ...
            if (Query.ContainsKey("v_updated_at"))
                updated_at.AdvancedSearch.SearchCondition = Get("v_updated_at");
            if (Query.ContainsKey("y_updated_at"))
                updated_at.AdvancedSearch.SearchValue2 = Get("y_updated_at");
            if (!Empty(updated_at.AdvancedSearch.SearchValue2) && Command == "")
                Command = "search";
            if (Query.ContainsKey("w_updated_at"))
                updated_at.AdvancedSearch.SearchOperator2 = Get("w_updated_at");
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (photo.UseColorbox) {
                    ...
                    photo.LinkAttrs["data-rel"] = "users_x" + RowCount + "_photo";
        ...
        protected void SetupSearchOptions() {
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"users\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("userssearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("userssearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
