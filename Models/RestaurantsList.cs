...
public partial class WebApp {
    ...
    /// restaurantsList
    /// </summary>
    public static RestaurantsList restaurantsList
    {
        get => HttpData.Get<RestaurantsList>("restaurantsList")!;
        set => HttpData["restaurantsList"] = value;
    }

    /// <summary>
    /// Page class for restaurants
    /// </summary>
    public class RestaurantsList : RestaurantsListBase
    {
        // Constructor
        public RestaurantsList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public RestaurantsList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class RestaurantsListBase : Restaurants
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "restaurantsList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "frestaurantslist";
        ...
        // Constructor
        public RestaurantsListBase()
        {
            // CSS class name as context
            TableVar = "restaurants";
            ...
            // Initialize URLs
            AddUrl = "restaurantsadd?" + Config.TableShowDetail + "=";
            ...
            MultiDeleteUrl = "restaurantsdelete";
            MultiUpdateUrl = "restaurantsupdate";
        ...
        // Page name
        public string PageName => "restaurantslist";
        ...
        // Constructor
        public RestaurantsListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "frestaurantssrch");
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "frestaurantssrch", filters);
        ...
        protected async Task SetupListOptions() {
            ...
            // "detail_dishes"
            item = ListOptions.Add("detail_dishes");
            item.CssClass = "text-nowrap";
            item.Visible = Security.AllowList(CurrentProjectID + "dishes");
            item.OnLeft = true;
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""restaurants"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""restaurants"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""restaurants"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"frestaurantslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"frestaurantslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            // "detail_dishes"
            listOption = ListOptions["detail_dishes"];
            isVisible = Security.AllowList(CurrentProjectID + "dishes");
            if (isVisible) {
                string caption, title, url;
                var body = Language.Phrase("DetailLink") + Language.TablePhrase("dishes", "TblCaption");
                body = "<a class=\"btn btn-default ew-row-link ew-detail" + (ListOptions.UseDropDownButton ? " dropdown-toggle" : "") + "\" data-action=\"list\" href=\"" + HtmlEncode(AppPath("disheslist?" + Config.TableShowMaster + "=restaurants&" + ForeignKeyUrl("fk_id", id.CurrentValue) + "")) + "\">" + body + "</a>";
                string links = "";
                detailPage = Resolve("DishesGrid")!;
                if (detailPage?.DetailView && Security.CanView && Security.AllowView(CurrentProjectID + "restaurants") ?? false) {
                    caption = Language.Phrase("MasterDetailViewLink", null);
                    title = Language.Phrase("MasterDetailViewLink", true);
                    url = GetViewUrl(Config.TableShowDetail + "=dishes");
                    ...
                    if (!Empty(detailViewTblVar))
                        detailViewTblVar += ",";
                    detailViewTblVar += "dishes";
                }
                if (detailPage?.DetailEdit && Security.CanEdit && Security.AllowEdit(CurrentProjectID + "restaurants") ?? false) {
                    caption = Language.Phrase("MasterDetailEditLink", null);
                    title = Language.Phrase("MasterDetailEditLink", true);
                    url = GetEditUrl(Config.TableShowDetail + "=dishes");
                    ...
                    if (!Empty(detailEditTblVar))
                        detailEditTblVar += ",";
                    detailEditTblVar += "dishes";
                }
                if (detailPage?.DetailAdd && Security.CanAdd && Security.AllowAdd(CurrentProjectID + "restaurants") ?? false) {
                    caption = Language.Phrase("MasterDetailCopyLink", null);
                    title = Language.Phrase("MasterDetailCopyLink", true);
                    url = GetCopyUrl(Config.TableShowDetail + "=dishes");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""restaurants"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item = option.Add("detailadd_dishes");
            dishes = Resolve("dishes")!;
            if (dishes != null) {
                caption = Language.Phrase("Add") + "&nbsp;" + Caption + "/" + dishes.Caption;
                title = Language.Phrase("Add", true) + "&nbsp;" + Caption + "/" + dishes.Caption;
                item.Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" + title + "\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(GetAddUrl(Config.TableShowDetail + "=dishes"))) + "\">" + caption + "</a>";
                item.Visible = (dishes.DetailAdd && Security.AllowAdd(CurrentProjectID + "restaurants") && Security.CanAdd);
                if (item.Visible) {
                    if (detailTableLink != "")
                        detailTableLink += ",";
                    detailTableLink += "dishes";
                }
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"frestaurantssrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"frestaurantssrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"frestaurantslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_restaurants");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(restaurantsList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(restaurantsList.RowCount) + "_restaurants");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(restaurantsList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && restaurantsList.RowType == RowType.Add || IsEdit && restaurantsList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (image.UseColorbox) {
                    ...
                    image.LinkAttrs["data-rel"] = "restaurants_x" + RowCount + "_image";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"frestaurantssrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
