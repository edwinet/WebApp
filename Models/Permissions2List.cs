...
public partial class WebApp {
    ...
    /// permissions2List
    /// </summary>
    public static Permissions2List permissions2List
    {
        get => HttpData.Get<Permissions2List>("permissions2List")!;
        set => HttpData["permissions2List"] = value;
    }

    /// <summary>
    /// Page class for permissions
    /// </summary>
    public class Permissions2List : Permissions2ListBase
    {
        // Constructor
        public Permissions2List(Controller controller) : base(controller)
        {
        }

        // Constructor
        public Permissions2List() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class Permissions2ListBase : Permissions2
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "permissions";

        // Page object name
        public string PageObjName = "permissions2List";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fpermissions2list";
        ...
        // Constructor
        public Permissions2ListBase()
        {
            // CSS class name as context
            TableVar = "permissions2";
            ...
            if (permissions2 == null || permissions2 is Permissions2)
                permissions2 = this;

            // Initialize URLs
            AddUrl = "permissions2add";
            ...
            MultiDeleteUrl = "permissions2delete";
            MultiUpdateUrl = "permissions2update";
        ...
        // Page name
        public string PageName => "permissions2list";
        ...
        // Constructor
        public Permissions2ListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "permissions2view" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("entity") ? dict["entity"] : entity.CurrentValue)) + "/";
            key += UrlEncode(ConvertToString(dict.ContainsKey("role_id") ? dict["role_id"] : role_id.CurrentValue));
            return key;
        }
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fpermissions2srch");
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fpermissions2srch", filters);
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""permissions2"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fpermissions2list\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fpermissions2list\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(entity.CurrentValue + Config.CompositeKeySeparator + role_id.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("role_id")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fpermissions2srch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fpermissions2srch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fpermissions2list\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_permissions2");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(permissions2List.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(permissions2List.RowCount) + "_permissions2");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(permissions2List.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && permissions2List.RowType == RowType.Add || IsEdit && permissions2List.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // role_id
                role_id.ViewValue = role_id.CurrentValue;
                role_id.ViewValue = FormatNumber(role_id.ViewValue, role_id.FormatPattern);
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fpermissions2srch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
