...
public partial class WebApp {
    ...
    /// dishesList
    /// </summary>
    public static DishesList dishesList
    {
        get => HttpData.Get<DishesList>("dishesList")!;
        set => HttpData["dishesList"] = value;
    }

    /// <summary>
    /// Page class for dishes
    /// </summary>
    public class DishesList : DishesListBase
    {
        // Constructor
        public DishesList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public DishesList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class DishesListBase : Dishes
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "dishesList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fdisheslist";
        ...
        public DishesListBase()
        {
            ...
            MultiDeleteUrl = "dishesdelete";
            MultiUpdateUrl = "dishesupdate";
        ...
        // Page name
        public string PageName => "disheslist";
        ...
        // Constructor
        public DishesListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            if (Config.SearchFilterOption == "Server")
                savedFilterList = await Profile.GetSearchFilters(CurrentUserName(), "fdishessrch");
            ...
            filters.Merge(JObject.Parse(image.AdvancedSearch.ToJson())); // Field image
            filters.Merge(JObject.Parse(price.AdvancedSearch.ToJson())); // Field price
            filters.Merge(JObject.Parse(restaurant_id.AdvancedSearch.ToJson())); // Field restaurant_id
            filters.Merge(JObject.Parse(created_at.AdvancedSearch.ToJson())); // Field created_at
            filters.Merge(JObject.Parse(updated_at.AdvancedSearch.ToJson())); // Field updated_at
            filters.Merge(JObject.Parse(created_by.AdvancedSearch.ToJson())); // Field created_by
            filters.Merge(JObject.Parse(updated_by.AdvancedSearch.ToJson())); // Field updated_by
        ...
        protected async Task<object?> ProcessFilterList() {
            if (Post("ajax") == "savefilters") {
                ...
                await Profile.SetSearchFilters(CurrentUserName(), "fdishessrch", filters);
        ...
        protected bool RestoreFilterList() {
            ...
            // Field image
            if (filter?.TryGetValue("x_image", out sv) ?? false) {
                image.AdvancedSearch.SearchValue = sv;
                image.AdvancedSearch.SearchOperator = filter["z_image"];
                image.AdvancedSearch.SearchCondition = filter["v_image"];
                image.AdvancedSearch.SearchValue2 = filter["y_image"];
                image.AdvancedSearch.SearchOperator2 = filter["w_image"];
                image.AdvancedSearch.Save();
            }
            ...
            // Field restaurant_id
            if (filter?.TryGetValue("x_restaurant_id", out sv) ?? false) {
                restaurant_id.AdvancedSearch.SearchValue = sv;
                restaurant_id.AdvancedSearch.SearchOperator = filter["z_restaurant_id"];
                restaurant_id.AdvancedSearch.SearchCondition = filter["v_restaurant_id"];
                restaurant_id.AdvancedSearch.SearchValue2 = filter["y_restaurant_id"];
                restaurant_id.AdvancedSearch.SearchOperator2 = filter["w_restaurant_id"];
                restaurant_id.AdvancedSearch.Save();
            }
            ...
            // Field updated_at
            if (filter?.TryGetValue("x_updated_at", out sv) ?? false) {
                updated_at.AdvancedSearch.SearchValue = sv;
                updated_at.AdvancedSearch.SearchOperator = filter["z_updated_at"];
                updated_at.AdvancedSearch.SearchCondition = filter["v_updated_at"];
                updated_at.AdvancedSearch.SearchValue2 = filter["y_updated_at"];
                updated_at.AdvancedSearch.SearchOperator2 = filter["w_updated_at"];
                updated_at.AdvancedSearch.Save();
            }
            ...
            // Field updated_by
            if (filter?.TryGetValue("x_updated_by", out sv) ?? false) {
                updated_by.AdvancedSearch.SearchValue = sv;
                updated_by.AdvancedSearch.SearchOperator = filter["z_updated_by"];
                updated_by.AdvancedSearch.SearchCondition = filter["v_updated_by"];
                updated_by.AdvancedSearch.SearchValue2 = filter["y_updated_by"];
                updated_by.AdvancedSearch.SearchOperator2 = filter["w_updated_by"];
                updated_by.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, description, def, false); // description
            BuildSearchSql(ref where, image, def, false); // image
            BuildSearchSql(ref where, price, def, false); // price
            BuildSearchSql(ref where, restaurant_id, def, false); // restaurant_id
            BuildSearchSql(ref where, created_at, def, false); // created_at
            BuildSearchSql(ref where, updated_at, def, false); // updated_at
            BuildSearchSql(ref where, created_by, def, false); // created_by
            BuildSearchSql(ref where, updated_by, def, false); // updated_by
            ...
            if (!def && Command == "search") {
                ...
                description.AdvancedSearch.Save(); // description
                image.AdvancedSearch.Save(); // image
                price.AdvancedSearch.Save(); // price
                restaurant_id.AdvancedSearch.Save(); // restaurant_id
                created_at.AdvancedSearch.Save(); // created_at
                updated_at.AdvancedSearch.Save(); // updated_at
                created_by.AdvancedSearch.Save(); // created_by
                updated_by.AdvancedSearch.Save(); // updated_by
        ...
        public void ShowFilterList()
        {
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, _name, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + _name.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field description
            filter = QueryBuilderWhere("description");
            if (Empty(filter))
                BuildSearchSql(ref filter, description, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + description.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field image
            filter = QueryBuilderWhere("image");
            if (Empty(filter))
                BuildSearchSql(ref filter, image, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + image.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field restaurant_id
            filter = QueryBuilderWhere("restaurant_id");
            if (Empty(filter))
                BuildSearchSql(ref filter, restaurant_id, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + restaurant_id.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected bool CheckSearchParms() {
            ...
            if (_name.AdvancedSearch.IssetSession)
                return true;
            if (description.AdvancedSearch.IssetSession)
                return true;
            if (image.AdvancedSearch.IssetSession)
                return true;
            if (price.AdvancedSearch.IssetSession)
                return true;
            if (restaurant_id.AdvancedSearch.IssetSession)
                return true;
            if (created_at.AdvancedSearch.IssetSession)
                return true;
            if (updated_at.AdvancedSearch.IssetSession)
                return true;
            if (created_by.AdvancedSearch.IssetSession)
                return true;
            if (updated_by.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            description.AdvancedSearch.UnsetSession();
            image.AdvancedSearch.UnsetSession();
            price.AdvancedSearch.UnsetSession();
            restaurant_id.AdvancedSearch.UnsetSession();
            created_at.AdvancedSearch.UnsetSession();
            updated_at.AdvancedSearch.UnsetSession();
            created_by.AdvancedSearch.UnsetSession();
            updated_by.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            description.AdvancedSearch.Load();
            image.AdvancedSearch.Load();
            price.AdvancedSearch.Load();
            restaurant_id.AdvancedSearch.Load();
            created_at.AdvancedSearch.Load();
            updated_at.AdvancedSearch.Load();
            created_by.AdvancedSearch.Load();
            updated_by.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(image); // image
                UpdateSort(price); // price
                UpdateSort(restaurant_id); // restaurant_id
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdisheslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fdisheslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("image")); // DN
                CreateColumnOption(option.Add("price")); // DN
                CreateColumnOption(option.Add("restaurant_id")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fdishessrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fdishessrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fdisheslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(dishesList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(dishesList.RowCount) + "_dishes");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(dishesList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && dishesList.RowType == RowType.Add || IsEdit && dishesList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // description
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_description"))
                    description.AdvancedSearch.SearchValue = Get("x_description");
                else
                    description.AdvancedSearch.SearchValue = Get("description"); // Default Value // DN
            if (!Empty(description.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_description"))
                description.AdvancedSearch.SearchOperator = Get("z_description");

            // image
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_image"))
                    image.AdvancedSearch.SearchValue = Get("x_image");
                else
                    image.AdvancedSearch.SearchValue = Get("image"); // Default Value // DN
            if (!Empty(image.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_image"))
                image.AdvancedSearch.SearchOperator = Get("z_image");
            ...
            // restaurant_id
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_restaurant_id"))
                    restaurant_id.AdvancedSearch.SearchValue = Get("x_restaurant_id");
                else
                    restaurant_id.AdvancedSearch.SearchValue = Get("restaurant_id"); // Default Value // DN
            if (!Empty(restaurant_id.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_restaurant_id"))
                restaurant_id.AdvancedSearch.SearchOperator = Get("z_restaurant_id");
            ...
            // updated_at
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_updated_at"))
                    updated_at.AdvancedSearch.SearchValue = Get("x_updated_at");
                else
                    updated_at.AdvancedSearch.SearchValue = Get("updated_at"); // Default Value // DN
            if (!Empty(updated_at.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_updated_at"))
                updated_at.AdvancedSearch.SearchOperator = Get("z_updated_at");
            ...
            // updated_by
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_updated_by"))
                    updated_by.AdvancedSearch.SearchValue = Get("x_updated_by");
                else
                    updated_by.AdvancedSearch.SearchValue = Get("updated_by"); // Default Value // DN
            if (!Empty(updated_by.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_updated_by"))
                updated_by.AdvancedSearch.SearchOperator = Get("z_updated_by");
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                _name.EditValue = HtmlEncode(_name.AdvancedSearch.SearchValue);
                _name.PlaceHolder = RemoveHtml(_name.Caption);

                // description
                description.SetupEditAttributes();
                description.EditValue = description.AdvancedSearch.SearchValue; // DN
                description.PlaceHolder = RemoveHtml(description.Caption);

                // image
                image.SetupEditAttributes();
                if (!image.Raw)
                    image.AdvancedSearch.SearchValue = HtmlDecode(image.AdvancedSearch.SearchValue);
                image.EditValue = HtmlEncode(image.AdvancedSearch.SearchValue);
                image.PlaceHolder = RemoveHtml(image.Caption);
                ...
                curVal = ConvertToString(restaurant_id.AdvancedSearch.SearchValue)?.Trim() ?? "";
                if (restaurant_id.Lookup != null && IsDictionary(restaurant_id.Lookup?.Options) && restaurant_id.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                    ...
                    if (curVal == "") {
                        ...
                        filterWrk = SearchFilter("[id]", "=", restaurant_id.AdvancedSearch.SearchValue, DataType.Number, "");
                    }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fdishessrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
        ...
        public async Task ExportData(dynamic? doc)
        {
            ...
            if (Config.ExportMasterRecord && !Empty(MasterFilterFromSession) && CurrentMasterTable == "restaurants") {
                restaurants = new RestaurantsList();
                if (restaurants != null) {
                    var c = await GetConnection2Async(restaurants.DbId); // Note: Use new connection for master record // DN
                    using var rsmaster = await restaurants.LoadReader(DbMasterFilter, "", c); // Load master record
                    if (rsmaster?.HasRows ?? false) { // DN
                        exportStyle = doc.Style;
                        doc.SetStyle("v"); // Change to vertical
                        if (!IsExport("csv") || Config.ExportMasterRecordForCsv) {
                            doc.Table = restaurants;
                            await restaurants.ExportDocument(doc, rsmaster, 1, 1);
                            doc.ExportEmptyRow();
                            doc.Table = this;
                        }
