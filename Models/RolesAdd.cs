...
public partial class WebApp {
    ...
    /// rolesAdd
    /// </summary>
    public static RolesAdd rolesAdd
    {
        get => HttpData.Get<RolesAdd>("rolesAdd")!;
        set => HttpData["rolesAdd"] = value;
    }

    /// <summary>
    /// Page class for roles
    /// </summary>
    public class RolesAdd : RolesAddBase
    {
        // Constructor
        public RolesAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public RolesAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class RolesAddBase : Roles
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "rolesAdd";
        ...
        // Page name
        public string PageName => "rolesadd";
        ...
        // Constructor
        public RolesAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("roleslist"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        else if (GetPageName(returnUrl) == "rolesview")
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            try {
                ...
                // role_name
                role_name.SetDbValue(rsnew, role_name.CurrentValue);

                // description
                description.SetDbValue(rsnew, description.CurrentValue);

                // type
                _type.SetDbValue(rsnew, _type.CurrentValue);
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
            ...
            if (id.Required && Empty(id.CurrentValue)) {
                FailureMessage = Language.Phrase("MissingUserLevelID");
            } else if (Empty(role_name.CurrentValue)) {
                FailureMessage = Language.Phrase("MissingUserLevelName");
            } else if (!IsNumeric(id.CurrentValue)) {
                FailureMessage = Language.Phrase("UserLevelIDInteger");
            } else if (ConvertToInt(id.CurrentValue) < -2) {
                FailureMessage = Language.Phrase("UserLevelIDIncorrect");
            } else if (ConvertToInt(id.CurrentValue) == 0 && !SameText(role_name.CurrentValue, "default")) {
                FailureMessage = Language.Phrase("UserLevelDefaultName");
            } else if (ConvertToInt(id.CurrentValue) == -1 && !SameText(role_name.CurrentValue, "administrator")) {
                FailureMessage = Language.Phrase("UserLevelAdministratorName");
            } else if (ConvertToInt(id.CurrentValue) == -2 && !SameText(role_name.CurrentValue, "anonymous")) {
                FailureMessage = Language.Phrase("UserLevelAnonymousName");
            } else if (ConvertToInt(id.CurrentValue) > 0 && (SameText(role_name.CurrentValue, "administrator") || SameText(role_name.CurrentValue, "default") || SameText(role_name.CurrentValue, "anonymous"))) {
